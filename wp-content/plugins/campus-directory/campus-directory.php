...
if (!class_exists('Campus_Directory')):
	...
	final class Campus_Directory {
		...
		public $textdomain = 'campus-directory';
		...
		private function define_constants() {
			define('CAMPUS_DIRECTORY_VERSION', '1.1.0');
		...
		private function includes() {
			...
			if (!class_exists('EDD_SL_Plugin_Updater')) {
				require_once CAMPUS_DIRECTORY_PLUGIN_DIR . 'assets/ext/edd/EDD_SL_Plugin_Updater.php';
			}
			if (!class_exists('Emd_License')) {
				require_once CAMPUS_DIRECTORY_PLUGIN_DIR . 'includes/admin/class-emd-license.php';
			}
			if (!function_exists('emd_show_license_page')) {
				require_once CAMPUS_DIRECTORY_PLUGIN_DIR . 'includes/admin/license-functions.php';
			}
			//the rest
			...
			if (!class_exists('Zebra_Form')) {
				require_once CAMPUS_DIRECTORY_PLUGIN_DIR . '/assets/ext/zebraform/Zebra_Form.php';
			}
			if (!function_exists('emd_submit_form')) {
				require_once CAMPUS_DIRECTORY_PLUGIN_DIR . 'includes/form-functions.php';
			}
			if (!function_exists('emd_submit_ajax_form')) {
				require_once CAMPUS_DIRECTORY_PLUGIN_DIR . 'includes/form-ajax-functions.php';
			}
			...
			if (!function_exists('emd_form_register_settings')) {
				require_once CAMPUS_DIRECTORY_PLUGIN_DIR . 'includes/admin/settings-functions-forms.php';
			}
			...
			require_once CAMPUS_DIRECTORY_PLUGIN_DIR . 'includes/forms.php';
		...
		public function load_plugin_textdomain() {
			...
			$mofile_shared = sprintf('%1$s-emd-plugins-%2$s.mo', 'campus-directory', $locale);
			$lang_file_list = Array(
				'emd-plugins' => $mofile_shared,
				'campus-directory' => $mofile
			);
			foreach ($lang_file_list as $lang_key => $lang_file) {
				$localmo = CAMPUS_DIRECTORY_PLUGIN_DIR . '/lang/' . $lang_file;
				$globalmo = WP_LANG_DIR . '/campus-directory/' . $lang_file;
				if (file_exists($globalmo)) {
					load_textdomain($lang_key, $globalmo);
				} elseif (file_exists($localmo)) {
					load_textdomain($lang_key, $localmo);
				} else {
					load_plugin_textdomain($lang_key, false, CAMPUS_DIRECTORY_PLUGIN_DIR . '/lang/');
				}
			}
		}
		/**
		...
		public function display_settings() {
			add_menu_page(__('CampusDirectory', $this->textdomain) , __('CampusDirectory', $this->textdomain) , 'manage_options', $this->app_name, array(
				$this,
		...
		public function display_support_page() {
			emd_display_support($this->textdomain, 2, 'campus-directory');
		}
