...
if (isset($_GET['campus_directory_optin'])) {
	if (!function_exists('wp_get_current_user')) {
		require_once (ABSPATH . 'wp-includes/pluggable.php');
	}
	if ($_GET['campus_directory_optin'] == 1) {
		//opt-in
	...
	wp_redirect(esc_url(remove_query_arg('campus_directory_optin')));
	exit;
} else {
...
function campus_directory_show_optin() {
	...
	if (!get_option('campus_directory_tracking_optin')) {
		...
		$tr_link = implode(' ', array(
			sprintf(__('%sDo not allow%s', 'campus-directory') , '<a href="' . admin_url('admin.php?page=campus_directory&campus_directory_optin=0') . '" class="button-secondary" id="campus-directory-do-not-allow-tracking">', '</a>') ,
			sprintf(__('%sAllow%s', 'campus-directory') , '<a href="' . admin_url('admin.php?page=campus_directory&campus_directory_optin=1') . '" class="button-primary" id="campus-directory-allow-tracking">', '</a>') ,
		));
		...
		echo '<li class="emd-permission"><i class="dashicons dashicons-email-alt"></i><div><span>' . __('Newsletter', 'campus-directory') . '</span><p>' . __('Updates, announcements, marketing, no spam', 'campus-directory') . ', <a href="https://emdplugins.com/subscription-preferences/" target="_blank">unsubscribe anytime</a></p></div></li>';
		echo '</ul></div></div>';
	} else {
...
function campus_directory_show_rateme_action() {
	...
	if (5 === $min_trigger) {
		...
		$response['redirect'] = "https://emdplugins.com/plugin_tag/campus-directory";
		$min_trigger = - 1;
	}
...
function campus_directory_show_rateme_notice() {
	...
	if ($min_count > 5 || ($min_trigger == 5 && $installed_date <= $today)) {
		$message_start = '<div class="emd-show-rateme update-nag success">
                        <span class=""><b>Campus Directory</b></span>
                        <div>';
		if ($min_count > 5) {
			$message_start.= sprintf(__("Hi, I noticed you just crossed the %d %s milestone on Campus Directory - that's awesome!", "campus-directory") , $min_trigger, $label);
		...
		$message_level1 = __('Could you please do me a <span style="color:red" class="dashicons dashicons-heart"></span> BIG favor <span style="color:red" class="dashicons dashicons-heart"></span> and give it a 5-star rating on WordPress? Just to help us spread the word and boost our motivation.', "campus-directory");
		...
		$message_end1 = '<li><a data-rate-action="do-rate" data-plugin="campus_directory" href="#">' . __('Ok, you deserve it', 'campus-directory') . '</a>
       </li>
        <li><a data-rate-action="done-rating" data-plugin="campus_directory" href="#">' . __('I already did', 'campus-directory') . '</a></li>
...
function campus_directory_plugin_row_meta($input, $file) {
	...
	$links = array(
		...
		'<a href="https://emdplugins.com/plugin_tag/campus-directory">' . __('Pro Version', 'campus-directory') . '</a>'
	);
...
function campus_directory_deactivation_feedback_box() {
	...
	foreach ($reasons as $key => $reason) {
		...
		switch ($key) {
			...
			case 9:
				$feedback_vars['reasons'].= 'data-input-type="textarea"';
				$feedback_vars['reasons'].= 'data-input-placeholder="' . __('We are sorry to hear that. Please share your previous version number before update, new updated version number and what happened', 'campus-directory') . '"';
			break;
			case 5:
				$feedback_vars['reasons'].= 'data-input-type="textarea"';
				$feedback_vars['reasons'].= 'data-input-placeholder="' . __('We are sorry to hear that. Please share what happened', 'campus-directory') . '"';
			break;
			case 22:
				$feedback_vars['reasons'].= 'data-input-type="textarea"';
