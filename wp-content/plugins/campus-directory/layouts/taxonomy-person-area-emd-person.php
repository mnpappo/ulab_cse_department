...
<div class="archive-well tax panel emd-person panel-<?php echo emd_get_attr_val('campus_directory', $post->ID, 'emd_person', 'emd_person_type', 'key'); ?>" style="background-color:#FFFFFF">
    <div class="panel-heading">
        <div class="panel-title"> <a class="archive permalink font-bold" href="<?php echo get_permalink(); ?>" title="<?php echo get_the_title(); ?>"><?php echo esc_html(emd_mb_meta('emd_person_fname')); ?>
        ...
        <div class="row">
            <div class="col-sm-4 <?php echo ((wp_get_attachment_url(current(get_post_meta(get_the_ID() , 'emd_person_photo')))) ? "well-left" : "well-left hidden"); ?>">
                <div class="slcontent emdbox">
                    ...
                    <div class="archive-image segment-block ent-person-photo"> <a href="<?php echo get_permalink(); ?>" title="<?php echo get_the_title(); ?>"><?php if (get_post_meta($post->ID, 'emd_person_photo')) {
		...
		echo '<img class="emd-img thumb" src="' . $thumb[0] . '" width="' . $thumb[1] . '" height="' . $thumb[2] . '" alt="' . get_post_meta($sval[0], '_wp_attachment_image_alt', true) . '"/>';
            ...
            <div class="col-sm-<?php echo ((wp_get_attachment_url(current(get_post_meta(get_the_ID() , 'emd_person_photo')))) ? "8" : "12"); ?>">
                <div class="srcontent emdbox">
                    ...
                    <div class="tax-content segment-block ent-person-type"> <?php echo emd_get_attr_val('campus_directory', $post->ID, 'emd_person', 'emd_person_type'); ?> </div>
                    <?php
} ?><?php if (emd_is_item_visible('tax_person_title', 'campus_directory', 'taxonomy')) { ?> 
                    <div class="tax-content segment-block tax-person-title"> <?php echo emd_get_tax_vals(get_the_ID() , 'person_title'); ?> </div>
                    <?php
} ?><?php if (emd_is_item_visible('ent_person_office', 'campus_directory', 'attribute')) { ?> 
                    <div class="tax-content segment-block ent-person-office"> <span class="dashicons dashicons-building"></span> <?php echo esc_html(emd_mb_meta('emd_person_office')); ?>
 </div>
                    <?php
} ?><?php if (emd_is_item_visible('ent_person_phone', 'campus_directory', 'attribute')) { ?> 
                    <div class="tax-content segment-block ent-person-phone"> <span class="dashicons dashicons-phone"></span> <a href="tel:<?php echo esc_html(emd_mb_meta('emd_person_phone')); ?>
"><?php echo esc_html(emd_mb_meta('emd_person_phone')); ?>
</a> </div>
                    <?php
} ?><?php if (emd_is_item_visible('ent_person_email', 'campus_directory', 'attribute')) { ?> 
                    <div class="tax-content segment-block ent-person-email"> <span class="dashicons dashicons-email-alt"></span> <a href="mailto:<?php echo antispambot(esc_html(emd_mb_meta('emd_person_email'))); ?>"><?php echo antispambot(esc_html(emd_mb_meta('emd_person_email'))); ?></a> </div>
                    <?php
} ?> 
                </div>
            </div>
