...
<?php 
	$has_sidebar = apply_filters( 'emd_show_temp_sidebar', 'right', 'campus_directory', 'single');
	if($has_sidebar ==  'left'){
...
<?php
	...
	while ( have_posts() ) : the_post(); ?>
		...
		<?php emd_get_template_part('campus-directory', 'single', str_replace("_","-",$post->post_type)); ?>
		</div>
		<?php
		...
		?>
<?php 	$has_navigation = apply_filters( 'emd_show_temp_navigation', true, 'campus_directory', 'single');
	if($has_navigation){
		global $wp_query;
