...
<?php 
	$has_sidebar = apply_filters( 'emd_show_temp_sidebar', 'right', 'campus_directory', 'archive');
	if($has_sidebar ==  'left'){
		do_action( 'emd_sidebar', 'campus-directory' );
	}
	if($has_sidebar == 'full'){
?>
...
<?php
	...
	while ( have_posts() ) : the_post(); ?>
			...
			<?php emd_get_template_part('campus-directory', 'archive', str_replace("_","-",$post->post_type)); ?>
			</div>
                <?php endwhile; // end of the loop. ?>
<?php	$has_navigation = apply_filters( 'emd_show_temp_navigation', true, 'campus_directory', 'archive');
	if($has_navigation){
		global $wp_query;
...
?>
<?php
	do_action( 'emd_sidebar', 'campus_directory' );
?>
<?php
}
?>
